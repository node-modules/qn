...
Qiniu.prototype.createError = function (statusCode, data) {
  ...
  switch (statusCode) {
  case 400:
    err.name = 'QiniuRequestParameterError';
    break;
  case 401:
    err.name = 'QiniuAuthFailError';
    break;
  case 405:
    err.name = 'QiniuRequestMethodWrongError';
    break;
  case 579:
    err.name = 'QiniuCallbackAppServerError';
    break;
  case 599:
    err.name = 'QiniuServerError';
    break;
  case 614:
    err.name = 'QiniuFileExistsError';
    break;
  case 631:
    err.name = 'QiniuBucketNotExistsError';
    break;
  case 701:
    err.name = 'QiniuDataChunkChecksumError';
    break;
  default:
...
Qiniu.prototype.upload = function upload(content, options, callback) {
  ...
  urllib.request(that._uploadURL, reqOptions, function (err, data, res) {
    ...
    if (statusCode >= 400) {
      err = that.createError(statusCode, data);
      return callback(err, data, res);
    }
